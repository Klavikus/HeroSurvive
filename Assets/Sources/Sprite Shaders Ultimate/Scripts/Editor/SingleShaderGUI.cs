    ...
    public class SingleShaderGUI : ShaderGUI
    {
        ...
        public static void DisplayProperties(ShaderInformation info, MaterialEditor materialEditor, MaterialProperty[] properties, int startIndex)
        {
            ...
            for(int n = 0; n < properties.Length; n++)
            {
                ...
                #region Reset
                if(propType != ShaderUtil.ShaderPropertyType.TexEnv && trueName.EndsWith("Toggle") == false && trueName != "Shader Space" && trueName != "Uber Fading" && trueName != "Time Settings" && prop.name != "_WindIsParallax")
                {
