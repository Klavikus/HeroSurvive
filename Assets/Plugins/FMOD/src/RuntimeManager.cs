...
namespace FMODUnity
{
    ...
    public class RuntimeManager : MonoBehaviour
    {
        ...
        public static void AttachInstanceToGameObject(FMOD.Studio.EventInstance instance, Transform transform,
            bool allowNonRigidBodyDoppler = false)
        {
            AttachedInstance attachedInstance =
        ...
        private IEnumerator loadFromWeb(string bankPath, string bankName, bool loadSamples)
        {
            ...
            loadResult = Instance.studioSystem.loadBankMemory(loadWebResult, FMOD.Studio.LOAD_BANK_FLAGS.NORMAL,
                out loadedBank.Bank);
            if (loadResult != FMOD.RESULT.OK)
            {
                RuntimeUtils.DebugLogWarningFormat("[FMOD] loadFromWeb.  Path = {0}, result = {1}.", bankPath,
                    loadResult);
            }
 ...
 false, System.Action completionCallback = null)
            ...
            GameObject attenuationObject = null)
        {
            if (attenuationObject)
            {
                ...
                (listenerIndex, gameObject.To3DAttributes(), attenuationObject.transform.position.ToFMODVector());
            }
            else
            {
                Vector3 unityPosition = gameObject.transform.position;
                Vector3 attenuationUnityPosition = attenuationObject.transform.position;
                var attributes3D = new ATTRIBUTES_3D()
                {
                    forward = gameObject.transform.up.ToFMODVector(),
                    position = new VECTOR() {x = unityPosition.x, y = 0, z = unityPosition.y}
                };
                
                Instance.studioSystem.setListenerAttributes(listenerIndex, attributes3D);
            }
        }
